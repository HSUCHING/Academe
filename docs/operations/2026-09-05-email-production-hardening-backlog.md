...
- [x] 调整品牌替换顺序，确保平台固定文案显示 Academe，同时不改写用户、课程或机构的真实名称。

当前 `apps/api/src/services/email/translations.py` 在 `.format(**fmt)` 后执行 `LearnHouse → Academe`。如果用户或机构真实名称包含 `LearnHouse`，动态值也会被改写。

完成记录（2026-09-05）：

- 品牌替换已移到 `.format(**fmt)` 之前，未修改任何语言字典或邮件调用方。
- 新增 `LearnHouse Research` 动态机构名回归断言；翻译目标测试 18/18、完整邮件与生命周期提醒测试 634/634 通过。
